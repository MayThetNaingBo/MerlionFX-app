...
def capture_payment():
    try:
        token = session['oauth_token']
        order_id = request.args.get('token')  # From PayPal return URL
        ...
        <p><strong>Name:</strong> {name_info.get("given_name", "")} {name_info.get("surname", "")}</p>
        <p><strong>Email:</strong> {email}</p>
        <p><strong>Amount:</strong> SGD {amount}</p>
        <p><strong>PayPal ID:</strong> {payer_id}</p>
        """
