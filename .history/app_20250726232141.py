...
def process_order():
    try:
        ...
        itemUnitAmount = Decimal(request.form['itemUnitAmount']).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
        ...
        # Save for use on capture page
        session['last_unit_price'] = f"{itemUnitAmount:.5f}"
        session['last_quantity'] = itemQuantity
...
def capture_payment():
    try:
        ...
        return render_template("order_result.html",
                               ...
                               fAmount=amount,
                               fPayPalID=payer_id,
                               fUnitPrice=session.get("last_unit_price", "N/A"),
                               fQuantity=session.get("last_quantity", "N/A"))
