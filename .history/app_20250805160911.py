...
def login():
    try:
        paypal = OAuth2Session(client_id, redirect_uri=redirect_uri,
            scope='openid profile email address https://uri.paypal.com/dashboard/paypalattributes')
...
def services():
    try:
        ...
        return render_template("dashboard.html", fName=user["name"], fEmail=user["email"],
...
def capture_payment():
    try:
        ...
        order = requests.post(url, headers=headers).json()

        info = order.get("payer", {}).get("name", {})
        return render_template("order_result.html", fOrderID=order["id"], fStatus=order["status"],
                               fName=info.get("given_name", ""), fSurname=info.get("surname", ""),
                               fEmail=order.get("payer", {}).get("email_address", ""),
                               fAmount=session.get("fx_total"), fPayPalID=payerID,
                               fUnitPrice=session.get("fx_unit_price"), fQuantity=session.get("fx_quantity"))
    except Exception as e:
        return str(e)
