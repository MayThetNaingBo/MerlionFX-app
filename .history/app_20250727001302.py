...
def process_order():
    try:
        ...
        itemUnitAmount = Decimal(request.form['itemUnitAmount']).quantize(Decimal('0.00001'))
        ...
        session["fx_unit_price"] = str(itemUnitAmount)
        session["fx_quantity"] = str(itemQuantity)
        session["fx_total_amount"] = str(totalAmount)
...
def capture_payment():
    try:
        ...
        unit_price = session.get("fx_unit_price", "N/A")
        quantity = session.get("fx_quantity", "N/A")
        total_amount = session.get("fx_total_amount", "N/A")
