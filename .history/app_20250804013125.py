...
@app.route("/api/fluctuation")
def fluctuation():
    from_ccy = request.args.get("from", "USD")
    to_ccy = request.args.get("to", "SGD")
    days = int(request.args.get("days", 7))

    end_date = datetime.today().strftime("%Y-%m-%d")
    start_date = (datetime.today() - timedelta(days=days)).strftime("%Y-%m-%d")

    url = f"https://api.exchangerate.host/fluctuation?start_date={start_date}&end_date={end_date}&base={from_ccy}&symbols={to_ccy}"
