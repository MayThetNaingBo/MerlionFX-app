...
def process_order():
    try:
        ...
        # Fix: Round unit amount to 2 decimals
        raw_unit_price = Decimal(request.form['itemUnitAmount'])
        itemUnitAmount = raw_unit_price.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)

        itemQuantity = int(request.form['itemQuantity'])

        # Fix: Calculate total based on 2-decimal-rounded unit price
        totalAmount = (itemUnitAmount * Decimal(itemQuantity)).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
        ...
        response = requests.post("https://api-m.sandbox.paypal.com/v2/checkout/orders", headers=headers, data=payload)
...
def capture_payment():
    try:
        ...
        unit_price = request.args.get("unit_price", "N/A")  # optional if passed along
